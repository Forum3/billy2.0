...
"""
Official Mem0 client integration.

This module provides a wrapper around the official Mem0 package
that maintains compatibility with the existing custom Mem0Client.
"""
...
class Mem0OfficialClient:
    """
    Client for interacting with the Mem0 memory system using the official package.
    ...
    It wraps the official Mem0 package while maintaining the same interface
    as the custom Mem0Client.
    """
    
    def __init__(self, api_key: str, agent_id: str, base_url: str = "https://api.mem0.ai", cache_ttl: int = 300):
        """
        Initialize the official Mem0 memory client.
        ...
        try:
            ...
            logger.error("Failed to import official Mem0 client. Make sure it's installed.")
            raise
        ...
        logger.info(f"Official Mem0 client initialized for agent {agent_id}")
    ...
    def add(self, memory_item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Convert to the format expected by the official client
            content = memory_item['content']
            metadata = memory_item.get('metadata', {})
            
            # Add priority if not present
            if "priority" not in metadata:
                metadata["priority"] = 5  # Default priority (1-10)
            else:
            ...
            # Format as messages for the MemoryClient
            messages = [
                {"role": "user", "content": content}
            ]
            
            # Add the memory using the agent_id as user_id
            ...
            # Format response to match custom client
            response_data = {
                'id': memory_id,
                'content': content,
                'metadata': metadata
    ...
    def search(self, search_params: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        Returns:
            List of dictionaries containing matching memory items
        """
        try:
            ...
            # Ensure limit is set if not provided
            limit = search_params.get('limit', 10)
            
            # Get query and ensure it's not blank (API requirement)
            query = search_params.get('query', '')
            if not query:
                query = "*"  # Use wildcard for all items
            
            # Convert metadata_filter to filters for compatibility
            filters = search_params.get('metadata_filter', search_params.get('filters', {}))
            ...
            for key, value in search_params.items():
                if key not in ['query', 'limit', 'metadata_filter', 'filters']:
    ...
    def update(self, memory_id: str, updates: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if 'content' in updates:
                ...
                if isinstance(updates["content"], str):
                    updates["messages"] = [
    ...
    def clear(self) -> bool:
        """
        Clear all memory items for the agent.
    ...
    def update_batch(self, updates: List[Dict[str, Any]]) -> List[Optional[Dict[str, Any]]]:
        ...
        try:
            ...
            for update in updates:
                ...
                del update_copy["id"]  # Remove ID from the update data
                ...
                # Add version information
                if 'metadata' not in update_copy:
                    update_copy['metadata'] = {}
                
                # Get current version or default to 0
                current_version = current.get('metadata', {}).get('version', 0)
                
                # Update version information
                update_copy['metadata']['version'] = current_version + 1
                update_copy['metadata']['previous_version'] = current_version
                
                # Store previous content if content is being updated
                if 'content' in update_copy:
                    update_copy['metadata']['previous_content'] = current.get('content', '')
                    
                    # Store timestamp of the update
                    update_copy['metadata']['updated_at'] = datetime.now().isoformat()
                    
                    # Format content as messages for compatibility with official client
                    if isinstance(update_copy["content"], str):
                        update_copy["messages"] = [
                            {"role": "user", "content": update_copy["content"]}
                        ]
            ...
            for update in updates:
                try:
                    ...
                    result = self.update(memory_id, update_copy)
                    results.append(result)
                except Exception as item_e:
